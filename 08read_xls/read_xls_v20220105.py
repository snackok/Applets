...
def open_xls(xls_path=os.getcwd()):
    ...
    # 变换treeview 颜色尝试，失败2022.1.7
    # _style = ttk.Style()
    # _style.map('Treeview', foreground=fixed_map('foreground'),background=fixed_map('background'))
    # _style.configure("Treeview", background="black", fieldbackground="black", foreground="white")
    ...
    # 设置单元格背景色  无效2022.1.6,但是换3.7版本又可以
    # print(tkinter.TkVersion)
    # _tree.tag_configure('XXX', background="#383838", foreground="yellow")
    # print(_tree.tag_configure('XXX'))
...
def debug_btn():
    print(f'\n\n')
    print(f'------------------调试信息--------------------------')
    print(_tree.item('I005', tags='1'))
    print(_tree.item('I005', option='tags')[0])
    print(_tree.set('I'+str(1).rjust(3,'0'),0,'xxx'))
    # _tree.set('I003',3,tags='yyy')
    # _tree.tag_configure('yyy', background="#380038", foreground="yellow")
    # _tree.set(_tree.identify_element(1, 1), None, "XXXX")
    print(f'-------------------调试信息结束----------------------')
    print(f'\n\n')
...
def save_book():
    ...
    ws = _xls_wb.create_sheet('change1')
    ws.append(_titles)
...
def evluate(p_list):
    ...
    # m_result = evlu_element(m_first_col)
    print("evluate结果为：\n" + str(re_list_evlu))
    return re_list_evlu
